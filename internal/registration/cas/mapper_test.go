...
func TestServiceMapperBuildServicePayload(t *testing.T) {
	t.Run("builds CAS payload", func(t *testing.T) {
		...
		assert.Equal(t, "^https://((?i)dev1[.]k3ces[.]localdomain)(:443)?/bluespice(/.*)?", service.ServiceID)
		assert.Equal(t, "", service.LogoutURL)
...
func TestServiceMapperHelperFunctions(t *testing.T) {
	t.Run("cover protocol and path handling helpers", func(t *testing.T) {
		...
		assert.Equal(t, "^https://((?i)dev1[.]k3ces[.]localdomain)(:443)?/app(/.*)?", pattern)
		...
		assert.False(t, regexp.MustCompile(pattern).MatchString("https://dev1.k3ces.localdomain:8443/app/api/v2/cas/auth"))
